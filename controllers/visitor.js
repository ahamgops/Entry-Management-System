...
module.exports.checkinVisitor = async (req, res) => {
    try {
        ...
        console.log(host);
        visitor.host_allotted = host._id;
...
module.exports.checkoutVisitor = async (req, res) => {
    try {
        const visitor = await Visitor.findOne({ email: req.body.email, checked_in: true }).populate("host_allotted");
        if (!visitor) {
            req.flash("error", "The visitor is not checked-in");
            return res.redirect("/visitor/checkout");
        }
        const host = await Host.findById(visitor.host_allotted);
        if (!host) {
